...
const Likes = () => {
  const { results, isSuccess, isFetchingNextPage, loadMoreButtonOnClick, hasNextPage } = useInfiniteData('likes', getLikes);
  return (
    <Layout>
      <NewsHeader>
        <NewsHeaderNav />
      ...
      <div>
        <NewsListBox>
          <StyledHeader className="posts-head">
            <h1>📙 읽었슴
              <span className="status">{isSuccess && results.length}개 읽었슴!</span>
            </h1>
          </StyledHeader>
          <StNewsList>
            {isSuccess && results.map((item) => (
              <NewsItem
                key={item._id}
                id={item.id}
                title={item.title}
